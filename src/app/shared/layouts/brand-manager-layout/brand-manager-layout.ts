...
const NAV_ITEMS: readonly NavItem[] = [
  { path: '/brand-manager/dashboard', label: 'Mi Resumen', icon: 'pi pi-home' },
  { path: '/brand-manager/sales', label: 'Ventas', icon: 'pi pi-shopping-cart' },
  { path: '/brand-manager/inventory', label: 'Mi Stock', icon: 'pi pi-box' },
  { path: '/brand-manager/settlements', label: 'Liquidaciones', icon: 'pi pi-wallet' },
];

@Component({
  selector: 'app-brand-manager-layout',
  ...
  template: `
    <div class="min-h-screen flex bg-surface-50 dark:bg-surface-900">
      ...
      >
        <div
          class="h-16 flex items-center px-6 border-b border-surface-200 dark:border-surface-700"
        >
        ...
        >
          ...
          <div class="text-right">
            <div class="text-sm font-medium text-surface-900 dark:text-surface-0">
              {{ user()?.fullName ?? 'BrandManager' }}
            </div>
            <div class="text-xs text-surface-500 dark:text-surface-400">Marca asociada</div>
          </div>
          <button
            pButton
            icon="pi pi-sign-out"
...
export class BrandManagerLayout {
